...
ARCH=$(uname -m)
if [[ "$ARCH" != "arm64" ]]; then
    echo -e "${INFO} Your system does not use Apple Silicon (ARM64). Please use the standard installer for Intel-based Macs."
    exit 0
fi

DYLIB_URL="https://x099xkycxe.ufs.sh/f/ar75CUBjeUn97lXG57giSg2Cb3OUYDHqn5ozMk0fmAtRrcsx"
...
EOF
    echo -e "${NC}"
    echo -e "${BLUE}=[ Opiumware (Apple Silicon) Installer ]=${NC}"
...
main() {
    ...
    run_step "Installing modules" bash -c "
        ...
        mv '$TEMP/libOpiumwareNative.dylib' '$APP_DIR/Roblox.app/Contents/Resources/libOpiumwareNative.dylib' &&

        curl -# -L '$MODULES_URL' -o '$TEMP/modules.zip' &&
        unzip -oq '$TEMP/modules.zip' -d '$TEMP' &&

        '$TEMP/Resources/Injector' \
            '$APP_DIR/Roblox.app/Contents/Resources/libOpiumwareNative.dylib' \
